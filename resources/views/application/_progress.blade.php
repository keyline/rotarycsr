...
<div class="mb-8 flex items-center gap-2 rounded-2xl border border-white/40 bg-white/90 px-4 py-4 shadow-sm backdrop-blur sm:px-6">
    @foreach ($steps as $i => $key)
        @php
            ...
            $isCompleted = $application->isSubmitted() || $num < $application->current_step;
        ...
        <div class="flex items-center gap-2 flex-1 last:flex-none">
            <div class="flex flex-col items-center gap-1">
                <div aria-label="{{ $title }}: {{ $stateLabel }}" class="h-8 w-8 rounded-full flex items-center justify-center text-xs font-semibold shrink-0
                    {{ $isCompleted ? 'bg-rotary-blue text-white' : ($isCurrent ? 'bg-rotary-gold text-rotary-navy ring-4 ring-rotary-gold/20' : 'bg-white text-gray-400 ring-1 ring-gray-200') }}"
                    @if ($isCompleted) style="background-color: #17458F;" @endif>
                    @if ($isCompleted)
            ...
            @if (! $loop->last)
                <div class="h-0.5 flex-1 {{ $isCompleted ? 'bg-rotary-blue' : 'bg-gray-200' }}"></div>
            @endif
        </div>
    @endforeach
</div>
